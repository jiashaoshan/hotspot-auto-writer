...
- **🔍 5大平台热点聚合**：微博/知乎/B站/小红书/Twitter（支持Cookie登录）
- **🤖 AI智能评估**：基于5维评估框架，并发5个评估，10个热点
- **✍️ 并发生成文章**：2篇文章同时生成，独立临时目录隔离
- **🚀 串行发布**：避免API限流，带实时进度和详细日志
- **🖼️ 智能封面**：Pexels真实图片优先，豆包AI生成备选
- **⚡ 高效省时**：全流程约 9-11 分钟，总流程提速30%+
...
- ✅ 添加微博/知乎Cookie登录支持
- ✅ AI评估并发优化（5并发评估，15→10热点，提速34%）
- ✅ 文章生成并行化（独立临时目录，隔离冲突）
- ✅ 发布串行化（避免API限流，5秒间隔）
- ✅ 封面Pexels优先策略（真实图片 > AI生成）
- ✅ YAML转义修复（解决特殊字符导致的发布超时）
- ✅ 总流程优化：11分钟→9分钟
